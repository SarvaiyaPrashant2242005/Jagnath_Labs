...
const ProvisionalQuotationForm = () => {
  ...
  const handleSelectTRF = async (trId) => {
    ...
    // Calculate regular quotation totals from TRF parameters & price masters
    let regSubtotal = 0;
    trParams.forEach(trp => {
      const pId = trp.parameterId || trp.parameter_id || trp.id;
      let paramPrice = parseFloat(trp.price);
      if (isNaN(paramPrice)) {
        const pm = (masters.priceMasters || []).find(p => p.parameterId === pId || p.parameter_id === pId);
        if (pm && !isNaN(parseFloat(pm.price))) {
          paramPrice = parseFloat(pm.price);
        } else {
          const paramObj = (masters.parameters || []).find(p => p.id === pId);
          if (paramObj && !isNaN(parseFloat(paramObj.price))) {
            paramPrice = parseFloat(paramObj.price);
          } else {
            paramPrice = 0;
          }
        }
      }
      regSubtotal += (paramPrice || 0);
    });
    ...
    triggerToast(`Loaded TRF ${reportNo} • ${paramCount} selected parameter(s) mapped!`, 'success');
  };
  ...
  const handleSave = (silent = false) => {
    ...
    try {
      if (quotationCategoryType === 'regular') {
        ...
        const regQuoNo = formData.quotationNumber && !formData.quotationNumber.startsWith('Q-P.I') && !formData.quotationNumber.includes('/EAC/')
          ? formData.quotationNumber
          : `JLT/EM/${selectedTR.reportNumber || selectedTR.sampleIdNumber || formData.referenceNo || '06-25/449'}`;

        const regGrandTotal = formData.regularGrandTotal || formData.grandTotal || (formData.regularSubtotal ? Math.round(formData.regularSubtotal * 1.18) : 0);
    ...
    <div className="provisional-quotation-module">
          ...
          <button
            ...
            onClick={() => {
              if (quotationCategoryType === 'regular') {
                ...
                const targetId = saved?.testRequestId || formData.testRequestId;
                window.open(`#/test-requests/quotation/${targetId}`, '_blank');
              } else {
                const saved = handleSave(true);
                const targetId = saved?.id || formData.id;
                window.open(`#/quotations/provisional/print/${targetId}`, '_blank');
              }
            }}
          >
      ...
      <div className="builder-split-view">
        ...
        <div className="builder-form-area" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          {/* Quotation Type Selector (Single Row Compact) */}
          <div style={{
            background: '#ffffff',
            ...
            <div style={{ background: '#f0f9ff', border: '1.5px solid #bae6fd', borderRadius: '10px', padding: '1.25rem', boxShadow: '0 2px 6px rgba(2, 132, 199, 0.08)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem' }}>
                <button
                  ...
                  style={{ background: '#0284c7', borderColor: '#0284c7', padding: '0.65rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
                >
                  <FaSave /> {saving ? 'Saving...' : 'Save Regular Quotation'}
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  style={{ padding: '0.65rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', background: '#ffffff' }}
                >
                  <FaPrint /> Print / View PDF
                </button>
              </div>
            </div>
          )}
